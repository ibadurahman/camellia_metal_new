                            ...
                            <table class="table table-striped table-valign-middle">
                                ...
                                <tbody>
                                    @foreach ($machines as $mc)
                                        <tr>
                                            <td>{{ $data[$mc->name]['machine'] }}</td>
                                            <td>{{ $data[$mc->name]['wo_number'] }}</td>
                                            <td>{{ $data[$mc->name]['size'] }}</td>
                                            <td>{{ $data[$mc->name]['customer'] }}</td>
                                            <td>{{ $data[$mc->name]['processedBy'] }}</td>
                                            <td>{{ $data[$mc->name]['start_time'] }}</td>
                                            <td class="@if ($data[$mc->name]['status'] == 'on process') text-success @endif">
                                                {{ $data[$mc->name]['status'] }}</td>
                                            <td>0 MPM</td>
                                            <td>0 PCS</td>
                                        </tr>
                                        <tr>
    ...
    <script>
        ...
        function updateSpeedChart() {
            ...
            $.ajax({
                ...
                dataType: 'json',
                success: function(response) {
                    document.getElementById('speedVal').innerHTML = response.speed +
                        '<sup style="font-size: 20px">MPM</sup>';
                    $('.speed').hide();
                    document.getElementById('counterVal').innerHTML = response.counter +
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#onprocess_data').html(
                        ...
                        '<b class="d-block">' + response.size + '</b>' +
                        '</p>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
                        ...
                        '<div class="card-body">' +
                        '<div class="text-muted">' +
                        '<p class="text-sm">Processed By' +
                        '<b class="d-block">' + response.processedBy + '</b>' +
                        '</p>' +
                        '<p class="text-sm">Start' +
                        '<b class="d-block">' + response.start_time + '</b>' +
                        '</p>' +
                        '</div>' +
                        '</div>' +
                        '</div>' +
